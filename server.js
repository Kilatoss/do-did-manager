...
const server = http.createServer(async (req, res) => {
  ...
  // Lidar com pedidos "Preflight" (OPTIONS)
  ...
  if (req.url === "/" && req.method === "GET") {
    ...
    fs.readFile(filePath, (err, content) => {
      if (err) {
        res.writeHead(500);
        res.end(
          "Erro: index.html não encontrado. Verifica se a pasta se chama 'public'."
        );
      } else {
      ...
      try {
        ...
        if (!user.salt || !user.hash) {
          ...
          res.end(
            JSON.stringify({
              success: false,
              message: "Formato de conta inválido. Registe-se novamente.",
            })
          );
          return;
        }
        ...
        if (derivedHash !== user.hash) {
          res.end(
            JSON.stringify({ success: false, message: "Password incorreta." })
          );
        } else {
  ...
  if (req.url.startsWith("/api/tasks/") && req.method === "PUT") {
    ...
    req.on("end", async () => {
      try {
        ...
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(
          JSON.stringify({ success: true, message: "Tarefa atualizada." })
        );
      } catch (error) {
